...
function fightsForRun(botName: BotName, seed: number): Fight[] {
  ...
  const fights: Fight[] = [];
  let cur: { encounter: number; enemyMaxHp: number; firstWordDamage: number; words: number } | null = null;
  for (let guard = 0; guard < 50000; guard++) {
    ...
    const pre =
      state.phase === 'fight' && state.encounter && actions.some((a) => a.type === 'submitWord')
        ? { encounter: state.encounterIndex + 1, enemyMaxHp: state.encounter.enemy.maxHp, rawDamage: Math.max(0, ...candidateWords(state, ctx).map((c) => c.damage)) }
        : null;
    for (const a of actions) state = reduce(state, a, ctx);
    if (state.rejected) throw new Error(`seed ${seed}: ${state.rejected}`);
    if (pre) {
      const dmg = pre.rawDamage;
      const killed = state.phase !== 'fight'; // a word that kills ends the fight (reward phase / summary)
      if (!cur) cur = { encounter: pre.encounter, enemyMaxHp: pre.enemyMaxHp, firstWordDamage: dmg, words: 0 };
      cur.words++;
      if (killed) {
        fights.push({ encounter: cur.encounter, enemyMaxHp: cur.enemyMaxHp, firstWordDamage: cur.firstWordDamage, wordsToKill: cur.words, oneShot: cur.words === 1 });
        cur = null;
      }
    }
  }
...
function main() {
  ...
  console.log('| enc | fights | one-shot % | overkill x (median) | overkill x (p90) | enemy HP | first-word dmg (median) |');
  console.log('|-----|--------|------------|---------------------|------------------|----------|-------------------------|');
  ...
  for (let e = 1; e <= maxEnc; e++) {
    ...
    const dmg = median(fs.map((f) => f.firstWordDamage));
    console.log(
      `| ${String(e).padStart(3)} | ${String(fs.length).padStart(6)} | ${((100 * oneShots) / fs.length).toFixed(1).padStart(10)} | ${median(ratios).toFixed(1).padStart(19)} | ${quantile(ratios, 0.9).toFixed(1).padStart(16)} | ${hp.toFixed(0).padStart(8)} | ${dmg.toFixed(0).padStart(23)} |`,
    );
  }
