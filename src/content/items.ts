/**
 * Placeholder items, named for the cell that carries them (Dean, 2026-09-08: word and
 * evolution themed, not generic). Ten items across five hooks and eight effect types:
 * enough to prove the hook system moves the sim, not to be balanced. Ids are stable; only
 * names and descriptions change, so saves and tests keyed by id survive renames.
 * Rarity weights: see RARITY_WEIGHT.
 */
import type { ItemDef, Rarity } from '../engine/types';

export const RARITY_WEIGHT: Readonly<Record<Rarity, number>> = { common: 3, uncommon: 2, rare: 1 };

export const ITEMS: readonly ItemDef[] = [
  {
    id: 'sharp-pen',
    name: 'Flagellum',
    rarity: 'common',
    description: '+5 damage on every word. A lash behind every syllable.',
    hooks: { onWordScored: [{ type: 'addFlat', value: 5 }] },
  },
  {
    id: 'lens',
    name: 'Photoreceptor',
    rarity: 'uncommon',
    description: '+50% damage. It sees where the weak spot is.',
    hooks: { onWordScored: [{ type: 'addMult', value: 0.5 }] },
  },
  {
    id: 'long-fuse',
    name: 'Ribosome',
    rarity: 'rare',
    description: 'Words of 6+ letters deal double damage. Long chains, long proteins.',
    hooks: {
      onWordScored: [{ type: 'condition', when: { kind: 'minLength', value: 6 }, then: [{ type: 'addMult', value: 1 }] }],
    },
  },
  {
    id: 'vowel-magnet',
    name: 'Vacuole',
    rarity: 'common',
    description: 'Vowels are drawn 50% more often. It stores what you keep running out of.',
    hooks: { onTileDraw: [{ type: 'vowelWeight', value: 1.5 }] },
  },
  {
    id: 'bandage',
    name: 'Mitosis',
    rarity: 'common',
    description: 'Heal 15 after each encounter. Split, recover, continue.',
    hooks: { onEncounterEnd: [{ type: 'heal', value: 15 }] },
  },
  {
    id: 'thick-skin',
    name: 'Membrane',
    rarity: 'uncommon',
    description: 'Take 3 less damage from every hit. Thicker than it looks.',
    hooks: { onDamageTaken: [{ type: 'reduceDamage', value: 3 }] },
  },
  {
    id: 'rare-ink',
    name: 'Enzyme',
    rarity: 'uncommon',
    description: 'J, Q, X and Z are worth +8 each. It catalyses the awkward letters.',
    hooks: { onWordScored: [{ type: 'letterBonus', letters: 'jqxz', value: 8 }] },
  },
  {
    id: 'leech',
    name: 'Cilia',
    rarity: 'uncommon',
    description: 'Heal 3 on every word of 5+ letters. Every long word sweeps in a little life.',
    hooks: {
      onWordScored: [{ type: 'condition', when: { kind: 'minLength', value: 5 }, then: [{ type: 'heal', value: 3 }] }],
    },
  },
  {
    id: 'spores',
    name: 'Spores',
    rarity: 'common',
    description: 'Deal 4 damage to the enemy at the start of each turn. They drift, they land, they burn.',
    hooks: { onTurnStart: [{ type: 'damageEnemy', value: 4 }] },
  },
  // Pool expansion (Dean, 2026-09-08: the first offer was always the same commons). Fourteen
  // more items from the existing vocabulary; rule-benders wait for new effect types.
  {
    id: 'lysosome',
    name: 'Lysosome',
    rarity: 'common',
    description: 'Words of 4 letters or fewer deal +6. Small, sharp, everywhere.',
    hooks: { onWordScored: [{ type: 'condition', when: { kind: 'maxLength', value: 4 }, then: [{ type: 'addFlat', value: 6 }] }] },
  },
  {
    id: 'pilus',
    name: 'Pilus',
    rarity: 'common',
    description: 'Words containing S deal +4. It grips.',
    hooks: { onWordScored: [{ type: 'condition', when: { kind: 'containsLetter', letters: 's' }, then: [{ type: 'addFlat', value: 4 }] }] },
  },
  {
    id: 'chloroplast',
    name: 'Chloroplast',
    rarity: 'common',
    description: 'Heal 1 at the start of every turn. Sunlight, slowly.',
    hooks: { onTurnStart: [{ type: 'heal', value: 1 }] },
  },
  {
    id: 'cell-wall',
    name: 'Cell Wall',
    rarity: 'common',
    description: 'While below half HP, take 4 less damage from every hit. It stiffens under stress.',
    hooks: { onDamageTaken: [{ type: 'condition', when: { kind: 'hpBelow', fraction: 0.5 }, then: [{ type: 'reduceDamage', value: 4 }] }] },
  },
  {
    id: 'nucleus',
    name: 'Nucleus',
    rarity: 'common',
    description: 'Every vowel in a word is worth +1. The centre holds.',
    hooks: { onWordScored: [{ type: 'letterBonus', letters: 'aeiou', value: 1 }] },
  },
  {
    id: 'plasmid',
    name: 'Plasmid',
    rarity: 'common',
    description: 'Every second turn, +30% damage. Borrowed genes, on loan.',
    hooks: { onWordScored: [{ type: 'condition', when: { kind: 'turnEvery', value: 2 }, then: [{ type: 'addMult', value: 0.3 }] }] },
  },
  {
    id: 'peroxisome',
    name: 'Peroxisome',
    rarity: 'uncommon',
    description: 'Words of 5+ letters deal +10. It breaks things down.',
    hooks: { onWordScored: [{ type: 'condition', when: { kind: 'minLength', value: 5 }, then: [{ type: 'addFlat', value: 10 }] }] },
  },
  {
    id: 'golgi',
    name: 'Golgi Body',
    rarity: 'uncommon',
    description: 'L, N, R, S and T are worth +2 each. It packages the ordinary.',
    hooks: { onWordScored: [{ type: 'letterBonus', letters: 'lnrst', value: 2 }] },
  },
  {
    id: 'centriole',
    name: 'Centriole',
    rarity: 'uncommon',
    description: 'Every third turn, deal 10 damage at the start of the turn. It pulls things apart.',
    hooks: { onTurnStart: [{ type: 'condition', when: { kind: 'turnEvery', value: 3 }, then: [{ type: 'damageEnemy', value: 10 }] }] },
  },
  {
    id: 'tentacle',
    name: 'Tentacle',
    rarity: 'uncommon',
    description: 'Words containing J, Q, X or Z deal double damage. Reach for the awkward ones.',
    hooks: { onWordScored: [{ type: 'condition', when: { kind: 'containsLetter', letters: 'jqxz' }, then: [{ type: 'addMult', value: 1 }] }] },
  },
  {
    id: 'symbiont',
    name: 'Symbiont',
    rarity: 'uncommon',
    description: 'Heal 25 after each encounter, but lose 1 HP at the start of every turn. It eats too.',
    hooks: { onEncounterEnd: [{ type: 'heal', value: 25 }], onTurnStart: [{ type: 'damagePlayer', value: 1 }] },
  },
  {
    id: 'apex',
    name: 'Apex Membrane',
    rarity: 'rare',
    description: 'Words of 7+ letters deal triple damage. The top of the food chain.',
    hooks: { onWordScored: [{ type: 'condition', when: { kind: 'minLength', value: 7 }, then: [{ type: 'addMult', value: 2 }] }] },
  },
  {
    id: 'regeneration',
    name: 'Regeneration',
    rarity: 'rare',
    description: 'Heal 3 whenever you take damage. It grows back.',
    hooks: { onDamageTaken: [{ type: 'heal', value: 3 }] },
  },
  {
    id: 'thin-membrane',
    name: 'Thin Membrane',
    rarity: 'rare',
    description: 'Double damage, but every hit you take hurts 4 more. Nothing between you and the world.',
    hooks: { onWordScored: [{ type: 'addMult', value: 1 }], onDamageTaken: [{ type: 'damagePlayer', value: 4 }] },
  },
  {
    id: 'second-wind',
    name: 'Cyst',
    rarity: 'rare',
    description: 'While below 30% HP, heal 6 whenever you take damage. Harden when it hurts.',
    hooks: {
      onDamageTaken: [{ type: 'condition', when: { kind: 'hpBelow', fraction: 0.3 }, then: [{ type: 'heal', value: 6 }] }],
    },
  },
];
