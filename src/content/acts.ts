...
export const TUNING: Tuning = {
  // Restored 2026-09-15 (Dean): the power-scaling wave flattened the top of this curve to kill the
  // stacker's one-shots, but that nerfed the PLAYER's power everywhere, dulled the long-word payoff
  // Dean likes ("endless feeling endless") and made even act 1 rough (act 1 enemies were untouched but
  // your words hit ~10% softer). We corrected the wrong side. Curve restored to its superlinear self;
  // the difficulty now rides on the enemy (the acts 2-3 wall below, and the consequence-scaling wave
  // that scales enemies to the player's build). superlinear from 5 so long words feel like events.
...
export const ENCOUNTERS: readonly EncounterDef[] = [
  ...
  // Power-scaling wave #106 (2026-09-14) raised these walls AND flattened lengthBonus; Dean found it
  // overcorrected (even act 1 rough, long-word payoff dulled). Reverted here (2026-09-15) to the exact
  // pre-#106 curve J, together with the lengthBonus restore above: back to the known "overpowered but
  // fun" baseline. The real difficulty work moves to the consequence-scaling wave (a research-backed
  // depth curve + a steeper endless race, no build-aware rubber-banding). See docs/exec-plans.
