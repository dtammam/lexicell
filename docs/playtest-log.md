...
| 2026-09-08 | Playtester: not sure what they are doing; the font creates too many ambiguous characters; what do the yellow tile, the light-blue border and the red border mean? Dean: do we reduce colours? Also "find" read as "And" | implemented (PR #47: tiles carry two signals, yellow vowel and pink rare edge; How to play card from the title with a real-value legend and the Readable type toggle; a first-fight hint line; ligatures off) |
| 2026-09-08 | Player: is there score significance to the yellow boxes and the coloured outlines, or is it visual? | implemented (PR #47: the How to play legend states each tier's point values; the first fight says it in one line) |
