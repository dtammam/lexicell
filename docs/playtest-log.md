...
| 2026-09-08 | A 200-item pool | in progress: scope confirmed and pack updated (PR #33); Dean agreed all six plan questions; engine PR #39 ships the nine verbs, seven conditions, onPick, offer rule and save v3; PR #40 ships the UI and batch 1 (112 items, curve D, criteria pass); batches 2-4 next |
...
| 2026-09-08 | Keyboard play on desktop: typing a letter that is on the grid selects a matching tile (optional feature, for keyboard players); do it only after the effects wave is merged | to do: after PRs #39 and #40 merge. Design: keydown on the fight screen selects the first unselected, unlocked tile with that letter (a venomed one first, since it wants spending), Backspace deselects the last tile, Enter attacks when the word is valid, Escape clears; no effect on touch devices; a hint line on desktop only |
