...
- **build 109 / PR #107** - Your big words are back (content tuning only: revert `TUNING.lengthBonus` to its superlinear curve; the acts 2-3 `ENCOUNTERS` wall from #106 stays). Dean playtested #108: it "overcorrected" and even act 1 was rough. Root cause: #106 flattened the length curve to kill the stacker's one-shots, but that nerfed the PLAYER's power everywhere (long-word payoff, the "endless feeling endless" fantasy) and made act 1 rough (act-1 enemies were untouched, but words hit ~10% softer). Corrected the wrong side. Restored the curve (len 7 back to 2.5, 10 to 4.0, 15 to 6.5); difficulty now rides on the enemy side (the kept acts 2-3 wall, and the consequence-scaling wave to follow). The `src/engine/evolution.test.ts` pre-boss baseline reverts to its exact pre-#106 hashes (reverting lengthBonus makes the driver replay the original act-1 words; this is the state PR #106's adversarial round already computed and validated, so no new reviewer round). Measured: `npm run power` one-shot 38.5% -> 57.2% (act 1 gentle again, big words back; acts 2-3 one-shots partly return, the consequence wave addresses them), overkill median 0.9x -> 1.1x. `npm run sim` (300) all three criteria pass: greedy 73.7% (< 90), mediocre 23.3% (band), solver 94.0%, no dead grids. `npm test`, `npm run lint` green. Content revert, iteration mode.
