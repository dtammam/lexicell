...
- **build 90 / PR #88** - The evolution track, the marquee wave (engine + save, v10 -> v11 with migration): the cell gains real CAPABILITIES as it descends, offered after each boss alongside the trait (phase 'capability', chained evolve -> trait -> capability -> item; skippable). Three first-class verbs, each Dean-approved: wildcard (the grid always carries one wild `?` tile, auto-resolved by the solver to the best-scoring letter; `solver.solveWithWild` accepts a word whose letter deficit against the non-wild letters is <= the wild count, a strict superset of `solve`, so a wild only ever adds words and the no-dead-grid guarantee holds), transmute (once per fight, turn a tile into the best rare letter K/J/X/Q/Z, deterministic, resets each encounter), and letter-bank (store a tapped tile's letter, spend it into a later word for scoring, per run). `RunState.evolution { caps, transmuteUsed, bankedLetter }` (JSON-plain) + `Tile.wild`; every wild code path is gated behind a present wild tile, so a run without the capability replays BYTE-IDENTICAL to main (golden hashes, seeds 0..11). Disclosed / OPEN for the coordinator: the wildcard raises the greedy ceiling (30-run smoke put greedy at 100%, over the <90 guard); the full 500-run balance sims and the bound decision (wild resolves to a common letter only, or one wild per fight) are the coordinator's, and the full adversarial gate (briefed to break replay) still runs. `npm test` 364/364, `npm run lint` clean, `scripts/sim.ts --runs 30` clean (no dead grids). Phone-render check of the mutation action row still owed.
